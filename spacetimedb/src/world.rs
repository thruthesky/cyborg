...
///
/// **그 뒤 3~10 에서 5~15 로 올렸다(사냥터를 5 할 더 붐비게 해 달라는 요청).**
/// 평균이 6.5 → 10 이므로 마릿수는 약 1.5 배(3,800 → 5,800 기)가 된다. 위에서
/// 서버가 무너졌던 22,500 기의 1/4 이고, 그 사이 백엔드가 maincloud 에서
/// 자체 VPS 로 옮겨 가며 AI 틱을 10 → 20 Hz 로 올릴 여유가 생긴 것을 확인한
/// 뒤였다([`MONSTER_AI_MICROS`]).
///
/// 최소값을 3 이 아니라 5 로 올린 것은 **성긴 군집을 없애기 위해서다.** 상한만
/// 올리면 평균은 오르지만 3 마리짜리 군집은 그대로 남아, 사냥터에 따라 붐비는
/// 정도가 들쭉날쭉해진다.
///
/// 화면이 성기게 느껴지면 이 값보다 **AI 틱 주기**([`MONSTER_AI_MICROS`])를
/// 먼저 살펴야 한다. 몹 수와 틱 빈도가 곱해져 부하가 되기 때문이다. 반대로
/// 서버가 밀리기 시작하면 여기부터 되돌린다.
pub const CLUSTER_MIN: u32 = 5;
pub const CLUSTER_MAX: u32 = 15;
...
///
/// 레벨 200 × 지역 3 × 최대 15 마리 = 9,000. 실제 마릿수는 군집마다 뽑은
/// 값의 합이라 이보다 적다(평균 10 마리이므로 약 6,000 기). 이 상수는 스폰 자리
/// 번호를 나눠 줄 때의 한계로만 쓴다.
...
/// 몬스터 AI 주기(마이크로초). **30 Hz** — 이 게임의 월드 틱이다.
///
...
const MONSTER_AI_MICROS: u64 = 33_333;
